...
parser.add_argument('--rectify_only', help='only generate rectified stereo images and stereo calibration in disparity mode', action='store_true')
...
def _load_pointcloud(kf, calib, ptcloud_name=None):
    if ptcloud_name is not None:
        return sio.load_ply_as_ptcloud(kf / ptcloud_name)

    try:
        return sio.load_scared_obj(kf / 'point_cloud.obj')
    except FileNotFoundError:
        return sio.load_scared_obj(kf / 'left_point_cloud.obj')


def _load_img3d_for_depth(kf, calib, ptcloud_name=None):
    if ptcloud_name is not None:
        gt_ptcloud = _load_pointcloud(kf, calib, ptcloud_name)
        return cvt.ptcloud_to_img3d(gt_ptcloud,
                                    calib['K1'],
                                    calib['D1'],
                                    (1024, 1280))

    try:
        return sio.load_img3d(kf / 'left_depth_map.tiff')
    except FileNotFoundError:
        gt_ptcloud = _load_pointcloud(kf, calib, None)
        return cvt.ptcloud_to_img3d(gt_ptcloud,
                                    calib['K1'],
                                    calib['D1'],
                                    (1024, 1280))
...
if __name__ == '__main__':
    ...
    if args.recursive:
        keyframe_dirs = sorted(
            [p for p in root_dir.rglob('**/keyframe*') if p.is_dir()],
            key=lambda p: str(p)
        )
    else:
    ...
    for kf in tqdm(keyframe_dirs, desc='keyframes processed'):
        ...
        if args.depth:
            try:
                gt_img3d = _load_img3d_for_depth(kf, calib, args.ptcloud)
            except FileNotFoundError:
                raise FileNotFoundError(
                    f"[{kf}] Missing depth sources: expected left_depth_map.tiff or point cloud files "
                    f"(point_cloud.obj/left_point_cloud.obj, or --ptcloud). "
                    f"For test datasets, use --disparity --rectify_only."
                )
        ...
        if args.undistort:
            try:
                gt_ptcloud = _load_pointcloud(kf, calib, args.ptcloud)
            except FileNotFoundError:
                raise FileNotFoundError(
                    f"[{kf}] Missing point cloud required for --undistort: expected "
                    f"point_cloud.obj/left_point_cloud.obj or --ptcloud file. "
                    f"For test datasets, use --disparity --rectify_only."
                )
        ...
        if args.disparity:
            ...
            try:
                ...
                raise FileNotFoundError(
                    f"[{kf}] Missing point cloud required to compute disparity/depthmap_rectified: expected "
                    f"point_cloud.obj/left_point_cloud.obj or --ptcloud file. "
                    f"Use --rectify_only to generate only rectified images and stereo_calib.json."
                )
